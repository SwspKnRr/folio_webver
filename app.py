    ...
    if st.button("티커 검색 실행", key="sym_search_btn"):
        if not query.strip():
            ...
            try:
                ...
                # auto-complete / search 둘 다 어느 정도 호환되게
                # - auto-complete 계열이면 q, region
                # - search 계열이면 q, quotesCount, newsCount도 무시 안 함
                params = {
                    "q": query,
                    "region": "US",
                ...
                # st.write("raw response:", data)  # 필요하면 잠깐 풀어서 구조 확인

                rows = []

                # 1️⃣ 기본: quotes에 심볼이 직접 들어오는 타입 (auto-complete / search 공통)
                quotes = data.get("quotes")
                if not quotes:
                    # 어떤 API는 ResultSet.Result 형태로 심볼을 줌
                ...
                # 2️⃣ quotes가 비어 있을 때: news의 relatedTickers에서 후보 뽑기
                if not rows:
                ...
                if not rows:
                    st.info("검색 결과가 없습니다. 영어 이름/심볼을 바꿔서 다시 시도해 보세요.")
                else:
